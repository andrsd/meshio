...
def write(filename, mesh):
    ...
    with netCDF4.Dataset(filename, "w") as rootgrp:
        ...
        rootgrp.version = np.float32(8.17)
        rootgrp.api_version = np.float32(8.17)
        rootgrp.floating_point_word_size = 8
        rootgrp.file_size = 1
        rootgrp.maximum_name_length = 32
        rootgrp.int64_status = 0

        _write_dimensions(mesh, rootgrp)
        _write_time_steps(mesh, rootgrp)
        _write_points(mesh, rootgrp)
        _write_cells(mesh, rootgrp)
        _write_point_data(mesh, rootgrp)
        _write_block_names(mesh, rootgrp)
        _write_side_sets(mesh, rootgrp)
        _write_side_set_names(mesh, rootgrp)
        _write_node_sets(mesh, rootgrp)
        _write_node_set_names(mesh, rootgrp)


def _write_dimensions(mesh, rootgrp):
    # set dimensions
    ...
    rootgrp.createDimension("num_side_sets", len(mesh.face_sets))
...
def _write_time_steps(mesh, rootgrp):
    # dummy time step
    data = rootgrp.createVariable("time_whole", "f8", ("time_step",))
    data[:] = 0.0


def _write_points(mesh, rootgrp):
    # points
    coor_names = rootgrp.createVariable("coor_names", "S1", ("num_dim", "len_string"))
    coor_names.set_auto_mask(False)

    crd_names = [b"X", b"Y", b"Z"]
    crd_var_names = ["coordx", "coordy", "coordz"]
    for dim in range(mesh.points.shape[1]):
        coor_names[dim, 0] = crd_names[dim]
        data = rootgrp.createVariable(
            crd_var_names[dim],
            numpy_to_exodus_dtype[mesh.points.dtype.name],
            ("num_nodes"),
        )
        data[:] = mesh.points[:, dim]


def _write_cells(mesh, rootgrp):
    # cells
...
def _write_point_data(mesh, rootgrp):
    # point data
...
def _write_block_names(mesh, rootgrp):
    data_names = rootgrp.createVariable("eb_names", "S1", ("num_el_blk", "len_name"))
    for k, name in enumerate(mesh.cell_set_names):
        for i, letter in enumerate(name):
            data_names[k, i] = letter.encode()


def _write_side_set_names(mesh, rootgrp):
    data_names = rootgrp.createVariable(
        "ss_names", "S1", ("num_side_sets", "len_string")
    )
    for k, name in enumerate(mesh.face_set_names):
        for i, letter in enumerate(name):
            data_names[k, i] = letter.encode()


def _write_side_sets(mesh, rootgrp):
    data = rootgrp.createVariable("ss_prop1", "i4", "num_side_sets")
    for k, (id, values) in enumerate(mesh.face_sets):
        data[k] = id
        dim1 = f"num_side_ss{k + 1}"
        rootgrp.createDimension(dim1, values.shape[0])
        dtype = numpy_to_exodus_dtype[values.dtype.name]
        data_el = rootgrp.createVariable(f"elem_ss{k + 1}", dtype, (dim1,))
        data_el[:] = values[:, 0] + 1
        dtype = numpy_to_exodus_dtype[values.dtype.name]
        data_sd = rootgrp.createVariable(f"side_ss{k + 1}", dtype, (dim1,))
        data_sd[:] = values[:, 1] + 1


def _write_node_set_names(mesh, rootgrp):
    data_names = rootgrp.createVariable(
...
def _write_node_sets(mesh, rootgrp):
    # node sets
